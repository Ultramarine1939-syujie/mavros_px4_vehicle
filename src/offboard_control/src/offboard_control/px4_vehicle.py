...
from offboard_control import px4_modes
from offboard_control import px4_builder
from offboard_control import px4_publisher
from offboard_control import ros_handle
...
class PX4Vehicle:
    ...
    def connect(self):
        ...
        if not self.is_connected():
            ...
            self.__offboard_pub = px4_publisher.OffboardPublisher(
                self.__topic_prefix)
    ...
    def return_to_launch(self, block = False):
        """
        Requests that the vehicle return
        """
        if self.is_armed():
            self.set_mode(px4_modes.PX4_MODE_RETURN)
            if block:
                self.wait_for_status(self.is_armed, False, 1)
        else:
            rospy.logerr("Vehicle is not armed.")
    ...
    def set_pose2d(self, pose2d_cmd, block = False, thres = 0.1):
        ...
        self.__offboard_pub.set_cmd(pose2d_cmd,
            px4_builder.CMD_SET_POSE_LOCAL)
